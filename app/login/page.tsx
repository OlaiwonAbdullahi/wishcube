...
const LoginPage: React.FC = () => {
  ...
  const handleGithubSignIn = async (): Promise<void> => {
    try {
      ...
      setErrors((prev) => ({
        ...prev,
        submit: "Failed to sign in with GitHub. Please try again.",
      }));
    }
  };
  ...
  useEffect(() => {
    if (user || googleUser || githubUser) {
      const currentUser = user?.user || googleUser?.user || githubUser?.user;
      if (currentUser && setUserProfile) {
        setUserProfile(currentUser);
      }
      router.push("/dashboard");
      toast.success("Signed in successfully");
    }
  }, [user, googleUser, githubUser, router, setUserProfile]);

  // Handle authentication errors
  useEffect(() => {
    const error = authError || googleError || githubError;
    if (error) {
  ...
  }, [authError, googleError, githubError]);
